...
thrBounds <- getThrBounds(dat, checkThr$newThr, 0.2)
plots <- checkThr$plot[1:4]
lpl <- length(plots)
for(colid in 1:ncol(dat)){
  ...
  plots[[colid+lpl]] <- ggplot(df.dp, aes(x=x, y=dp))+geom_point(size=0.1)+
    ...
    xlab("Partition ID")+
    ylab("Intensity")+
    geom_hline(yintercept=checkThr$newThr[colid],
               colour=checkThr$qc[colid], linewidth=1)+
    ...
    geom_hline(yintercept=thr[colid],
